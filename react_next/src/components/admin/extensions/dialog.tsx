...
const DialogNodeView: React.FC<NodeViewProps> = ({ node, updateAttributes, deleteNode }) => {
  ...
  return (
    <NodeViewWrapper>
      <div className={`dialog ${isStudent ? 'student' : ''}`}
        style={{
          margin: '14px 0', padding: '14px 16px', background: '#f8f6ef', borderRadius: 10,
          border: '1px solid #e0d8c8', display: 'flex', alignItems: 'flex-start', gap: 12, position: 'relative',
          transition: 'box-shadow 0.15s',
        }}
        onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.boxShadow = '0 0 0 2px #b8d4e8' }}
        onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.boxShadow = 'none' }}
      >
      <img src={isStudent ? TARO_IMG : PANDA_IMG} alt={name}
        style={{ width: 48, height: 48, flexShrink: 0, display: 'block' }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <strong style={{ color: '#8a7a5a', fontSize: 12.5, display: 'block', marginBottom: 2 }}>{name}</strong>
        <div contentEditable suppressContentEditableWarning
          onBlur={handleBlur} onKeyDown={handleKeyDown}
          style={{ outline: 'none', minHeight: 24, fontSize: 14, lineHeight: 1.6, color: '#333', cursor: 'text', borderRadius: 4, padding: '2px 4px', transition: 'background 0.12s' }}>
          {text || 'セリフを入力'}
        </div>
      </div>
      <button onClick={(e) => { e.stopPropagation(); deleteNode() }}
        style={{
          position: 'absolute', top: 4, right: 6, border: 'none', background: 'rgba(0,0,0,0.05)',
          cursor: 'pointer', color: '#999', fontSize: 16, width: 24, height: 24, borderRadius: 12,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          opacity: 0, transition: 'opacity 0.15s',
        }}
        className="dialog-delete-btn" title="削除">×</button>
      </div>
    </NodeViewWrapper>
  )
}
