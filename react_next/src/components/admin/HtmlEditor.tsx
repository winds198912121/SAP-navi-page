...
export default function HtmlEditor({ value: initialValue, onChange, placeholder }: HtmlEditorProps) {
  ...
  const editor = useEditor({
    extensions: [
      ...
      ImageExtension, Subscript, Superscript,
  ...
  useEffect(() => {
    ...
    if (initialValue !== contentRef.current) {
      ...
      if (mode === 'visual') editor.commands.setContent(initialValue)
      else if (mode === 'html') setHtmlSource(initialValue)
      else try { setMdSource(turndownService.turndown(initialValue)) } catch { setMdSource(initialValue) }
    }
  ...
  const handleModeChange = useCallback((newMode: EditorMode) => {
    ...
    if (newMode === 'html') {
      const html = formatHtml(contentRef.current); setHtmlSource(html); onChange(contentRef.current)
    } else if (newMode === 'markdown') {
      try { const md = turndownService.turndown(contentRef.current); setMdSource(md) } catch { setMdSource(contentRef.current) }
      onChange(contentRef.current)
    } else if (newMode === 'visual') {
      const html = mode === 'html' ? htmlSource : (marked.parse(mdSource, { async: false }) as string)
      editor.commands.setContent(html); contentRef.current = html; onChange(html)
    }
    setMode(newMode); setColorPicker(null)
    if (newMode !== 'visual') setTimeout(() => textAreaRef.current?.focus(), 50)
  ...
  const handleImageUpload = useCallback(() => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (json.success && json.data?.url) editor.chain().focus().setImage({ src: json.data.url }).run()
  ...
  const handleHtmlChange = (val: string) => { setHtmlSource(val); contentRef.current = val; onChange(val) }
  const handleMdChange = (val: string) => {
    setMdSource(val)
    try { const html = marked.parse(val, { async: false }) as string; contentRef.current = html; onChange(html) } catch {}
  }

  const insertHtmlAtCursor = (html: string) => {
    const ta = textAreaRef.current
    if (!ta) { handleHtmlChange(htmlSource + '\n' + html + '\n'); return }
    const start = ta.selectionStart; const end = ta.selectionEnd
    const newVal = htmlSource.substring(0, start) + html + htmlSource.substring(end)
    handleHtmlChange(newVal)
    requestAnimationFrame(() => { ta.setSelectionRange(start + html.length, start + html.length); ta.focus() })
  }

  const toggleColorPicker = (type: 'text' | 'bg') => setColorPicker(prev => prev === type ? null : type)
  ...
  function formatHtml(html: string): string {
    const tab = '  '; let result = ''; let indent = 0
    const pres: string[] = []
    ...
    return result.replace(/\x00PRE(\d+)\x00/g, (_, i) => pres[parseInt(i)]).trim()
  }
  ...
  const tb = (label: string, action: () => void, active?: boolean) => {
    ...
    return (<button key={label} type="button" onClick={action} title={label}
      style={{ padding: '4px 8px', fontSize: 13, fontWeight: isOn ? 700 : 400, cursor: 'pointer', border: 'none', background: isOn ? '#d4e4f7' : 'transparent', color: isOn ? '#1a5b9c' : '#444', borderRadius: 4, fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'all 0.1s' }}
      onMouseEnter={e => { if (!isOn) e.currentTarget.style.background = '#eee' }}
      onMouseLeave={e => { if (!isOn) e.currentTarget.style.background = 'transparent' }}>{label}</button>)
  }

  useEffect(() => { return () => { editor?.destroy() } }, [editor])

  const tabStyle = (active: boolean) => ({ padding: '8px 18px', fontSize: 12.5, fontWeight: active ? 700 : 500, cursor: 'pointer' as const, border: 'none', background: active ? 'var(--bg-card)' : 'transparent', color: active ? 'var(--accent-deep)' : 'var(--ink-2)', borderBottom: active ? '2px solid var(--accent)' : '2px solid transparent', fontFamily: 'inherit' as const, transition: 'all 0.12s' })
  const darkBtn: React.CSSProperties = { padding: '4px 10px', fontSize: 11.5, background: '#2a2b3e', color: '#a9b1d6', border: '1px solid #3a3b4e', borderRadius: 4, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }
  const styles = { bar: { display: 'flex', gap: 0, marginBottom: 0, borderBottom: '1px solid var(--line-2)', background: 'var(--bg-1)' } as React.CSSProperties, textarea: { width: '100%', minHeight: 360, padding: 16, fontFamily: "'JetBrains Mono', 'SF Mono', 'Consolas', monospace", fontSize: 13, lineHeight: 1.7, border: 'none', outline: 'none', resize: 'vertical', background: '#1a1b26', color: '#a9b1d6', tabSize: 2 } as React.CSSProperties }

  return (
    <div className="html-editor" style={{ border: '1px solid var(--line-2)', borderRadius: 'var(--r-md)', overflow: 'hidden' }}>
      <div style={styles.bar}>
        <button style={tabStyle(mode === 'visual')} onClick={() => handleModeChange('visual')} type="button">✏️ ビジュアル</button>
        <button style={tabStyle(mode === 'html')} onClick={() => handleModeChange('html')} type="button">{'</>'} HTML</button>
        <button style={tabStyle(mode === 'markdown')} onClick={() => handleModeChange('markdown')} type="button">📝 Markdown</button>
        <span style={{ marginLeft: 'auto', padding: '8px 14px', fontSize: 11, color: 'var(--ink-3)' }}>{mode === 'visual' ? 'TipTap WYSIWYG' : mode === 'html' ? 'HTML' : 'Markdown'}</span>
      </div>
      ...
      {mode === 'visual' && editor && (
        <div>
          <div style={{ display: 'flex', gap: 4, padding: '6px 10px', background: '#fcf8ee', borderBottom: '1px solid rgba(60,45,20,0.15)', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span style={{ flex: 1 }} />
            <span style={{ fontSize: 11, color: '#aaa', marginRight: 8 }}>吹き出し内のテキストをクリックして編集</span>
          </div>

          <div style={{ display: 'flex', gap: 2, padding: '6px 8px', background: '#f8f8f8', borderBottom: '1px solid #e0e0e0', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              {colorPicker === 'text' && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 100, background: '#fff', border: '1px solid #ddd', borderRadius: 8, padding: 6, display: 'flex', flexWrap: 'wrap', gap: 2, width: 168, boxShadow: '0 4px 12px rgba(0,0,0,0.12)' }}>
                  {TEXT_COLORS.map(c => (<button key={c} type="button" onClick={() => applyColor('text', c)} style={{ width: 24, height: 24, background: c, border: '1px solid #ddd', borderRadius: 4, cursor: 'pointer' }} title={c} />))}
                </div>
              )}
            </div>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              {colorPicker === 'bg' && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 100, background: '#fff', border: '1px solid #ddd', borderRadius: 8, padding: 6, display: 'flex', flexWrap: 'wrap', gap: 2, width: 168, boxShadow: '0 4px 12px rgba(0,0,0,0.12)' }}>
                  {BG_COLORS.map(c => (<button key={c} type="button" onClick={() => applyColor('bg', c)} style={{ width: 24, height: 24, background: c, border: '1px solid #ddd', borderRadius: 4, cursor: 'pointer' }} title={c} />))}
                </div>
              )}
            </div>
            <div style={{ width: 1, height: 20, background: '#ddd', margin: '0 4px' }} />
          ...
          <div style={{ padding: 0, minHeight: 340, cursor: 'text' }} onClick={() => editor.commands.focus()}>
            <style>{`.tiptap-editor .ProseMirror { min-height: 340px; padding: 14px 16px; outline: none; line-height: 1.85; font-size: 14px; font-family: "Noto Sans JP", system-ui, sans-serif; }
...
.tiptap-editor .ProseMirror blockquote { border-left: 3px solid #ccc; margin: 8px 0; padding: 4px 12px; color: #666; font-style: italic; }
.tiptap-editor .ProseMirror pre { background: #1a1b26; color: #a9b1d6; padding: 12px; border-radius: 6px; font-family: "JetBrains Mono", monospace; font-size: 13px; overflow-x: auto; margin: 8px 0; }
.tiptap-editor .ProseMirror code { font-family: "JetBrains Mono", monospace; font-size: 0.9em; background: #f0f0f0; padding: 1px 4px; border-radius: 3px; }
      ...
      {mode === 'html' && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#1a1b26', borderBottom: '1px solid #2a2b3e' }}>
            ...
            <button type="button" onClick={() => setHtmlSource(formatHtml(htmlSource))} style={{ padding: '4px 12px', fontSize: 11.5, background: '#2a2b3e', color: '#a9b1d6', border: '1px solid #3a3b4e', borderRadius: 4, cursor: 'pointer', fontFamily: 'inherit' }}>🔄 フォーマット</button>
          </div>
          <textarea ref={textAreaRef} style={styles.textarea} value={htmlSource} onChange={e => handleHtmlChange(e.target.value)} placeholder="HTML を直接入力..." spellCheck={false} />
        </div>
      )}

      {mode === 'markdown' && (
        <textarea ref={textAreaRef} style={styles.textarea} value={mdSource} onChange={e => handleMdChange(e.target.value)} placeholder="Markdown で入力（自動的に HTML に変換されます）..." spellCheck={false} />
      )}
    </div>
  )
}
