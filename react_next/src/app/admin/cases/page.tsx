  ...
  return (
    <>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'var(--spacing-xl)'}}>
        <h1>💼 案件管理</h1>
        <Link href="/admin/cases/new" className="btn btn-primary btn-sm">＋ 新規作成</Link>
      </div>
      {items.length > 0 ? (
        <table className="admin-table">
          ...
          <tbody>{items.map((item: any) => (
            <tr key={item.id}>
              ...
              <td className="actions"><Link href={`/admin/cases/${item.id}/edit`} className="btn btn-sm btn-ghost">編集</Link></td>
            </tr>
          ))}</tbody>
        </table>
