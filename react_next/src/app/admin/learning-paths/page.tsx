...
async function AdminLearningPaths() {
  const items = (await api.getLearningPaths({ per_page: 50 })) || [];
  return (
    <>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'var(--spacing-xl)'}}>
        <h1>🎯 学習パス管理</h1>
        <Link href="/admin/learning-paths/new" className="btn btn-primary btn-sm">＋ 新規作成</Link>
      </div>
      {items.length > 0 ? (
        <table className="admin-table">
          ...
          <tbody>{items.map((item: any) => (
            <tr key={item.id}>
              ...
              <td className="actions"><Link href={`/admin/learning-paths/${item.id}/edit`} className="btn btn-sm btn-ghost">編集</Link></td>
            </tr>
          ))}</tbody>
        </table>
