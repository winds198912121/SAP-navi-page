...
const COUNT_CARDS = [
  ...
  { key: 'paths', label: '学習パス', icon: '🎯', color: '#a855f7', bg: '#f3e8ff', link: '/admin/learning-paths' },
  { key: 'contact_unread', label: '未読問合せ', icon: '📨', color: '#ef4444', bg: '#fee2e2', link: '/admin/contact' },
]
        ...
        {COUNT_CARDS.map(card => {
          ...
          return (
            <Link key={card.key} href={card.link}
              style={{ background: card.bg, textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 6, padding: '16px 14px', borderRadius: 14, position: 'relative', overflow: 'hidden', transition: 'transform 0.12s', cursor: 'pointer' }}
              ...
              <div>
                <div style={{ fontFamily: "'Zen Maru Gothic',sans-serif", fontWeight: 800, fontSize: 24, color: card.color, lineHeight: 1.2 }}>
                  ...
                  {card.key === 'contact_unread' && val > 0 && (
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#ef4444', marginLeft: 4, verticalAlign: 'super' }}>NEW</span>
                  )}
                </div>
                <div className="admin-stat-label" style={{ color: '#4a4030' }}>{card.label}</div>
              </div>
            </Link>
          )
        })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {stats.recent_articles.map((a: any) => (
                <Link key={a.id} href={`/admin/articles/${a.id}/edit`}
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 8, textDecoration: 'none', color: 'inherit', fontSize: 13 }}
                  ...
                  <span style={{ fontSize: 11, color: 'var(--ink-3)', whiteSpace: 'nowrap', marginLeft: 12 }}>👁 {a.views?.toLocaleString() || 0}</span>
                </Link>
              ))}
            </div>
          )}
          <Link href="/admin/articles" style={{ display: 'block', marginTop: 10, fontSize: 12, color: '#3e7a52' }}>すべての記事を見る →</Link>
        </div>
          ...
          {(stats.popular_articles || []).length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {stats.popular_articles.map((a: any, i: number) => (
                <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px', borderRadius: 8, fontSize: 13 }}>
                  <span style={{ fontFamily: "'Zen Maru Gothic',sans-serif", fontWeight: 800, fontSize: 16, color: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#d97548' : 'var(--ink-3)', width: 24, textAlign: 'center' }}>{i + 1}</span>
        ...
        <div className="admin-form-card">
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            📨 最近のお問い合わせ
            {c.contact_unread > 0 && (
              <span style={{ fontSize: 10.5, padding: '2px 8px', borderRadius: 999, background: '#ef4444', color: 'white', fontWeight: 700 }}>{c.contact_unread}件未読</span>
            )}
          </h3>
          {(stats.recent_inquiries || []).length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {stats.recent_inquiries.map((inq: any) => (
                <Link key={inq.id} href="/admin/contact"
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 8, textDecoration: 'none', color: 'inherit', fontSize: 13 }}
                  onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-tint)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: inq.status === 'unread' ? '#ef4444' : inq.status === 'replied' ? '#5a9d6e' : '#94a3b8' }} />
                    {inq.name}
                  </span>
                  <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>{inq.created_at?.slice(0, 10)}</span>
                </Link>
              ))}
            </div>
          )}
          <Link href="/admin/contact" style={{ display: 'block', marginTop: 10, fontSize: 12, color: '#3e7a52' }}>すべて見る →</Link>
        </div>
          ...
          {(stats.recent_users || []).length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {stats.recent_users.map((u: any) => (
                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px', borderRadius: 8, fontSize: 13 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--ink-3)', whiteSpace: 'nowrap' }}>{u.registered_at?.slice(0, 10)}</span>
                </div>
              ))}
            </div>
          )}
          <Link href="/admin/users" style={{ display: 'block', marginTop: 10, fontSize: 12, color: '#3e7a52' }}>すべてのユーザーを見る →</Link>
        </div>
      </div>
    </div>
  )
