...
async function AdminCourses() {
  const items = (await api.getCourses({ per_page: 50 })) || [];
  return (
    <>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'var(--spacing-xl)'}}>
        <h1>📚 コース管理</h1>
        <Link href="/admin/courses/new" className="btn btn-primary btn-sm">＋ 新規作成</Link>
      </div>
      {items.length > 0 ? (
        <table className="admin-table">
          ...
          <tbody>{items.map((item: any) => (
            <tr key={item.id}>
              ...
              <td className="actions"><Link href={`/admin/courses/${item.id}/edit`} className="btn btn-sm btn-ghost">編集</Link></td>
            </tr>
          ))}</tbody>
        </table>
