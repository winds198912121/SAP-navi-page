...
$items    = aladdin_api_get( 'lessons', [ 'per_page' => 50 ] ) ?: [];
get_header(); ?>
<div class="admin-layout"><aside class="admin-sidebar"><?php include ALADDIN_THEME_DIR . '/page-templates/admin/_sidebar.php'; ?></aside>
<main class="admin-content"><div class="admin-header"><h1>📖 レッスン管理</h1><a href="/admin/lessons/new" class="btn btn-primary btn-sm">＋ 新規作成</a></div>
<?php if ( $sub_view === 'new' || ( $sub_view === 'edit' && $item_id ) ) : $item = $sub_view === 'edit' ? aladdin_api_get( 'lessons/' . $item_id ) : []; ?>
<form method="post" class="admin-form">
  ...
  <button type="submit" class="btn btn-primary">保存</button><a href="/admin/lessons" class="btn btn-ghost">キャンセル</a>
</form>
<?php else : ?>
...
<td class="actions"><a href="/admin/lessons/<?php echo esc_attr( $it['id'] ?? 0 ); ?>/edit" class="btn btn-sm btn-ghost">編集</a></td></tr><?php endforeach; ?></tbody></table>
<?php endif; ?></main></div>
<?php get_footer(); ?>
